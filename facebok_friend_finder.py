...
LOG_FILENAME = 'FacebookLogging.txt'

# Set up a specific logger with our desired output level
my_logger = logging.getLogger('FacebookFinder')
...
def facebook_login(username,password):
	...
	if "Please re-enter your password" in html_source or "Forgotten password" in html_source or "doesn't match any account" in html_source:
...
def extract_mutual_friends(profiles_urls):
	...
	for profile_url in profiles_urls:
		...
		isClickedOnMutual=False
		while True:

			time.sleep(2)
			try:
				#elem = driver.find_element_by_xpath(".//a[@class='pam uiBoxLightblue uiMorePagerPrimary']") 
				#elem = driver.find_element_by_xpath('//*[@id="js_14"][1]') - TO chrome
				#elem = driver.find_element_by_xpath('/html/body/div[1]/div[2]/div[1]/div/div[2]/div[2]/div[1]/div/div/div/span[2]/span[2]/a')#js_c
				if isClickedOnMutual is False:
# 					driver.execute_script('document.querySelector(\'#js_c\')')
# 					elem = driver.find_element_by_css_selector('#js_c')# ('.//*[@id="js_c"]')
					elem = driver.find_element_by_xpath('/html/body/div[1]/div[2]/div[1]/div/div[2]/div[2]/div[1]/div/div/div/span[2]/span[2]/a')
					friendsFrmWhen = driver.find_element_by_xpath('/html/body/div[1]/div[2]/div[1]/div/div[2]/div[2]/div[1]/div/div/div/span[1]/span[2]')
					print profile_url + "  " + friendsFrmWhen.text + "  " + elem.text
					elem.click()
					isClickedOnMutual=True
				else:
					isSeeMoreFound=True
					previousElement=None
					while isSeeMoreFound:
						isSeeMoreFound=False
						elements = driver.find_elements_by_xpath(".//a[@class='pam uiBoxLightblue uiMorePagerPrimary']")
						for elem in elements:
							if elem.text.lower() == 'see more':
								if not(None is previousElement) and elem.__eq__(previousElement):
									elem.click()
								else:
...
def save_file(filename,results):
	if args.profilesfile:
		line_items = 2
		with open(filename, 'w') as myFile , open('checkProfile','w') as checkProfile :
			for n, user in enumerate(results):
				if (n+1) % line_items:
					...
					checkProfile.write(user.encode('utf8')+"\n")
				else:
...
def find_him(profiles_urls):
	my_logger.debug("Entered Into findHim..")
	data = list()
	for profile_url in profiles_urls:
		elem = driver.get(profile_url)
		my_logger.debug("opening link of: "+profile_url)
		try:
					elem = driver.find_element_by_xpath('/html/body/div[1]/div[2]/div[1]/div/div[2]/div[2]/div[1]/div/div/div/span[2]/span[2]/a')
					my_logger.info(" Mutual : " + elem.text)
					friendsFrmWhen = driver.find_element_by_xpath('/html/body/div[1]/div[2]/div[1]/div/div[2]/div[2]/div[1]/div/div/div/span[1]/span[2]')
					my_logger.info( " Friends from : " + friendsFrmWhen.text )
		except NoSuchElementException as e:
				my_logger.error( 'NoSuchElementException Occured')
				my_logger.error(e)
				my_logger.debug(traceback.print_exc())
					
		except Exception as e:
				my_logger.error( 'Exception Occured')
				my_logger.error(e)
				my_logger.debug(traceback.print_exc())
